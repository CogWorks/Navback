;;;;;;;;;;DEBUG;;;;;;;;;;
(defparameter *process-spawn-count* 0)
;;;;;;;;;;/DEBUG;;;;;;;;;
...
;(require "MultiWorld")
(defparameter *mw-task-object* nil)

(in-package :cl-user)

(defparameter *run-time* 300)        ;set to 1200 for 20 minutes
(defparameter *prac-run-time* 300)    ;set to 120 for 2 minutes for practice block    
(defparameter *modality* 1)          ;1 = Unimodal;    2 = Multimodal;     3 = Persistent Unimodal
(defparameter *dirtype* 1)           ;1 = dir box  2 = in arrow
(defparameter *nback* 2)     
(defparameter *condition* 1)        ;1  1-back  2 = 3-back
...
;(defparameter *dirs-list* nil)
;(defparameter *users-turns* nil)
...
(capi:define-interface drive-interface1 ()
  ((arrow :initform (gp:read-external-image (merge-pathnames "mnt/arrow-green.bmp" (get-load-path)) :transparent-color-index 42) :accessor arrow)
   ...
   (arrow-x :initform nil :accessor arrow-x)
   (arrow-y :initform nil :accessor arrow-y)
...
(setf *mw-task-object* (register-task "Driving Task" :run-function 'drive :configure-function 'display-config-win))
(define-logging-folder (current-pathname))

(defun display-config-win ()
  (let ((condition (capi:prompt-with-list (list "V-DB-N1" "V-DB-N3" "V-A-N1" "V-A-N3" "A-N1" "A-N3") "Please choose a condition: ")))
    ...
    (configuration-done *mw-task-object* :condition condition)
    )
...
(defmethod start-drive-begin ((interface capi:interface))
...
(defun quit-drive (interface)
  ...
  (log-info (list "SNT-EVENT" "SNT-SUMMARY" *tm* *condition* *block* *deviation-score* *total-deviation-score* *perc-correct-turns* *total-perc-correct-turns*))
...
(defun scores (interface)
         ...
         (cond ((< *block* 9)
                 (capi:display-message "Your Scores for BLOCK ~A ~%~%Deviation Score: ~A ~%Percentage of Correct Turns: ~A ~%" *block* (round *deviation-score*) *perc-correct-turns*)
         ...
         ;(setf *tot-list-arrow-jitter* nil)
               ...
               ((eql *block* 9)
                 ...
                 (capi:display-message "Your TOTAL Scores~%~%Total Deviation Score: ~A ~%Total Percentage of Correct Turns: ~A" (round *total-deviation-score*) *total-perc-correct-turns*)
                
                 )
                (task-finished *mw-task-object*))
...
; create jitter-count variable which will determine the frequency of the jitter update function
    
      
     ; loop experiment re-drawing process until block time runs out
    (while (< (get-universal-time) *tm*)

; increment counts and sleep for 1 millisecond

           (sleep .1)

           (if (< (drive-speed interface) 1)
               (new-intersection interface)
             )
...
; test to see if it is time to either update the display or run the jitter function.  Log a step after each new display
      ...
      (capi:execute-with-interface
       ...
       #'(lambda ()
           (let ((new-image (gp:convert-external-image (road interface) image)))
             ...
             (capi:apply-in-pane-process (road interface) 'gp:draw-image (road interface) new-image x y)
...
(defun calc-perc-turns (interface)
  (let ((count 0)
    ...
    ;(setf *dirs-list* (reverse (dirs-list interface)))
    ;(setf *users-turns* (reverse (users-turns interface)))
    ...
    ;;update turn count
    ;(cond ((eql (turn-count interface) 3)
     ;      (setf (turn-count interface) 1))
     ;     ((not (eql (turn-count interface) 3))
     ;      (incf (turn-count interface)))
      ;    )

    )
)

(defun update-displays (interface)
  (let ((temp (if (eql (loc-perc-correct-turns interface) 100)
                  'yes
                'no)))
   (capi:apply-in-pane-process (deviation-score interface) #'(setf capi:display-pane-text) (format nil "~S" (round (loc-deviation-score interface))) (deviation-score interface))
 ...
 (setf (direction1 interface) (get-directions))
           ...
           (cond ((and (eql *modality* 1) (eql *nback* 2) (eql *dirtype* 2))
                 ...
                 ((and (eql *modality* 2) (eql *nback* 2))
                  (objc:with-autorelease-pool ()
                    (mp:process-run-function "Auditory" '() #'cw-speak (format nil "~S" (direction1 interface)) :voice 1))
                  ;;;DEBUG;;;
                  (incf *process-spawn-count*)
                  ...
                  (objc:with-autorelease-pool ()
                    (mp:process-run-function "Auditory" '() #'cw-speak (format nil "~S" (direction2 interface)) :voice 1))
                  ;;;DEBUG;;;
                  (incf *process-spawn-count*)
                  ;;;DEBUG;;;
                  ...
                  (objc:with-autorelease-pool ()
                    (mp:process-run-function "Auditory" '() #'cw-speak (format nil "~S" (direction3 interface)) :voice 1))
                  ;;;DEBUG;;;
                  (incf *process-spawn-count*)
                  ;;;DEBUG;;;
  ...
  (let ((pane (visual-directions interface)) (dir-to-present nil))
    (setf (direction1 interface) (direction2 interface))
           ...
           (cond ((and (eql *modality* 1) (eql *dirtype* 1))
                 ...
                 ((eql *modality* 2)
                  (objc:with-autorelease-pool ()
                    (mp:process-run-function "Auditory" '() #'cw-speak (format nil "~S" dir-to-present) :voice 1))
                  ;;;DEBUG;;;
                  (incf *process-spawn-count*)
                  ;;;DEBUG;;;
                  )
...
;(load (current-pathname "mnt/model-template.lisp"))
