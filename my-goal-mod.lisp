...
(defstruct (threaded-goal-module (:include goal-module) (:conc-name tgm-))
  chunk-set update ref-goal)


(defun cs-hook (lst)
  
  (aif (get-production-by-name (first lst))
       (setf (tgm-ref-goal (get-module goal)) (member 'goal (production-lhs-buffers it))))
  nil)
   ...
   (verify-current-model
    ...
    (let ((buffer (buffer-instance buffer-name)))
      (cond ((null buffer)
            ...
            ((not (and (eql buffer-name 'goal) (null (tgm-ref-goal (get-module goal)))))
...
(defun thread-clear (instance buffer chunk)
  (when (and (eq buffer 'goal) (find chunk (tgm-chunk-set instance)) (tgm-ref-goal instance)) 
...
(defun thread-search (instance buffer)
  ...
  (when (tgm-update instance) 
    ...
    (setf (tgm-update instance) nil))
  ;(format +actr-output+ "~%chunk set ~S" (tgm-chunk-set instance))
